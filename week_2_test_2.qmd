...
data <- data |> 
  drop_na() |> 
  mutate(Petal.Width = scale(Petal.Width),
...
# k = 2
```{r}
res <- resample_function(data = data, number_of_resamples = 20, k = 2)
final <- res |> 
  reduce(`+`)
...
# k = 3
```{r}
res <- resample_function(data = data, number_of_resamples = 20, k = 3)
final <- res |> 
  reduce(`+`) |> 
  as.vector() 
